...
If [`tidb_plan_replayer_external_storage_uri`](/system-variables.md#tidb_plan_replayer_external_storage_uri-new-in-v856) is configured, TiDB also stores captured files in the specified external storage. The `token` column in the `mysql.plan_replayer_status` table records the file token of each captured file.
